...
class SJMCARTUPSELL extends HTMLElement {
  ...
  initSwiper() {
    ...
    this.swiperInstance = new Swiper('.cart-upsell-swiper', {
      slidesPerView: 1,
      spaceBetween: 12,
      loop: false,
      centeredSlides: false,
      breakpoints: {
        320: { slidesPerView: 1.25 }, // Show 25% of next slide
        768: { slidesPerView: 1.25 }
      }
    });
  }

  // Replace inner content with fresh HTML
  refresh(html) {
    const parser = new DOMParser();
    const doc = parser.parseFromString(html, 'text/html');

    const newContent = doc.querySelector('sjm-cart-upsell');
...
document.addEventListener('cart:updated', () => {
  const upsellEl = document.querySelector('sjm-cart-upsell');
  if (!upsellEl) return;

  fetch('/?section_id=cart-upsell')
    .then(res => res.text())
    .then(html => {
      upsellEl.refresh(html);
    });
});
